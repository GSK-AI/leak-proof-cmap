...
def _percent_replicating_worker_init(
    df,
    features,
    replicate_query,
    replicate_criteria,
    ...
    global _pr_df, _pr_features, _pr_replicate_query, _pr_replicate_criteria, _pr_replicate_criteria_not, _pr_null_query_or_df, _pr_null_criteria, _pr_null_criteria_not, _pr_similarity_metric, _pr_n_iters, _pr_min_cardinality, _pr_max_cardinality, _pr_return_full_profiles, _pr_percentile_cutoff
    _pr_df = df
    _pr_features = features
    _pr_replicate_query = replicate_query
    _pr_replicate_criteria = replicate_criteria
    _pr_replicate_criteria_not = replicate_criteria_not
    _pr_null_query_or_df = null_query_or_df
    _pr_null_criteria = null_criteria
    _pr_null_criteria_not = null_criteria_not
    _pr_similarity_metric = similarity_metric
    _pr_n_iters = n_iters
    _pr_min_cardinality = min_cardinality
    _pr_max_cardinality = max_cardinality
    _pr_return_full_profiles = return_full_performance_df
    _pr_percentile_cutoff = percentile_cutoff
...
def _inspect_replicating_worker(
    # df: pd.DataFrame,
    # features: list[str],
    g_indices: list[int],
    # replicate_criteria: list[str],
    ...
    """Private function for multiprocessing workers to calc pr"""
    global _pr_df, _pr_features, _pr_replicate_query, _pr_replicate_criteria, _pr_replicate_criteria_not, _pr_null_query_or_df, _pr_null_criteria, _pr_null_criteria_not, _pr_similarity_metric, _pr_n_iters, _pr_min_cardinality, _pr_max_cardinality, _pr_return_full_performance_df, _pr_return_full_profiles, _pr_percentile_cutoff

    matching_multiindex, matching_indices = g_indices
    replicate_frame = _pr_df.iloc[matching_indices]

    # Early exit if required, before further checks
    if len(matching_indices) < 2 or len(matching_indices) < _pr_min_cardinality:
        return _ReplicateData(
            matching_multiindex, len(matching_indices), np.nan, np.nan
        )

    if _pr_replicate_query is not None:
        replicate_frame = replicate_frame.query(_pr_replicate_query)

    # If replicate_criteria_not contains fields not to match on, then we must regenerate 'matching_indices'
    if len(_pr_replicate_criteria_not) > 0:
        matching_indices = [
            choice(nmi)
            for nmi in [
                i_idx
                for i_idx in replicate_frame.groupby(
                    _pr_replicate_criteria_not
                ).indices.values()
            ]
        ]
        replicate_frame = replicate_frame.iloc[matching_indices]
    ...
    if cardinality == 1:
        return _ReplicateData(matching_multiindex, cardinality, np.nan, np.nan)
    # If less than required cardinality, then return as non-replicating
    if cardinality < _pr_min_cardinality:
        return _ReplicateData(matching_multiindex, cardinality, np.nan, np.nan)

    # If matching cardinality is > max_cardinality, then resample to max_cardinality
    if cardinality > _pr_max_cardinality:
        replicate_frame = replicate_frame.sample(n=_pr_max_cardinality)
        cardinality = _pr_max_cardinality
    ...
    # additional terms, like being in the same well as the matched compound. Or
    # in the case of null_criteria_not, ensuring things do not match.
    null_criteria_query_list = [f"{_pr_replicate_criteria[0]}!=@matching_multiindex[0]"]
    if len(_pr_null_criteria) > 0:
        tmp_matched_cpd_row = replicate_frame.iloc[0]
        for gbn in _pr_null_criteria:
            tmp_val = tmp_matched_cpd_row[gbn]
            if isinstance(tmp_val, (float, int)):
                null_criteria_query_list.append(f"{gbn}=={tmp_val}")
            else:
                null_criteria_query_list.append(f"{gbn}=='{tmp_val}'")
    # Generate non-matching queries
    if len(_pr_null_criteria_not) > 0:
        tmp_matched_cpd_row = replicate_frame.iloc[0]
        for gbn in _pr_null_criteria_not:
            tmp_val = tmp_matched_cpd_row[gbn]
            if isinstance(tmp_val, (float, int)):
                null_criteria_query_list.append(f"{gbn}!={tmp_val}")
            else:
    ...
    if isinstance(_pr_null_query_or_df, pd.DataFrame):
        null_df = _pr_null_query_or_df.query(" and ".join(null_criteria_query_list))
    else:
        null_df = _pr_df.query(" and ".join(null_criteria_query_list))
        if isinstance(_pr_null_query_or_df, str):
            null_df = null_df.query(_pr_null_query_or_df)

    # generate a dictionary (pert_name_to_idx_dict), allowing mapping from pert_name
    # to indexes (0 -indexed np array)
    null_pert_names = null_df[_pr_replicate_criteria[0]]
    pert_name_to_idx_dict = {pname: [] for pname in null_pert_names.unique()}
    for i, pert_name in enumerate(null_pert_names):
        pert_name_to_idx_dict[pert_name].append(i)
    if (
        len(pert_name_to_idx_dict) < 2
        or len(pert_name_to_idx_dict) < _pr_min_cardinality
    ):
        return _ReplicateData(
            matching_multiindex, len(matching_indices), np.nan, np.nan
        )
    ...
    # Calculate matched repeat score
    if _pr_similarity_metric == "spearman":
        matching_median_dist = np.median(
            np.corrcoef(np.argsort(np.argsort(replicate_frame[_pr_features].values)))[
                np.triu_indices(cardinality, k=1)
            ]
        )
    else:
        matching_median_dist = np.median(
            pdist(replicate_frame[_pr_features].values, metric=_pr_similarity_metric)
        )
    ...
    # Extract the profiles then reahape to a 3D array n_iters x cardinality x features
    profiles = (
        null_df[_pr_features]
        .values[indices]
        .reshape(_pr_n_iters, cardinality, len(_pr_features))
    )
    ...
    if _pr_similarity_metric == "spearman":
        null_distribution = np.array(
            [
                ...
                for p in range(_pr_n_iters)
            ]
        )
    else:
        null_distribution = np.array(
            [
                np.median(pdist(profiles[p], metric=_pr_similarity_metric))
                for p in range(_pr_n_iters)
            ]
        )
    ...
    return _ReplicateData(
        ...
        (
            null_distribution
            if _pr_return_full_profiles
            else np.percentile(null_distribution, float(_pr_percentile_cutoff))
        ),
    )
...
def percent_replicating(
    ...
    start_groupby = datetime.datetime.now()
    print(f"Performing groupby ({start_groupby})", end="")
    ...
    print(
        f"... completed groupby. Elapsed time = {datetime.datetime.now()-start_groupby}"
    )
    ...
    if parallel:
        with Pool(
            cpu_count() if n_jobs == -1 else n_jobs,
            initializer=_percent_replicating_worker_init,
            initargs=(
                df,
                features,
                replicate_query,
            ...
            for pr_res in pool.imap(
                _inspect_replicating_worker,
                tqdm(
                    [
                        g_indices
